...
看到這裡，你可能會有個疑問，為什麼這個 Express 路由沒有回傳一個 response？實際上是有的，只是被 `passport.authenticate` 給處理了，變成我們自己不需要去處理跟撰寫。當我們在上面使用 `passport.authenticate` 的時候，`passport` 會組出一個 URL 回傳 response 給瀏覽器要瀏覽器轉導致 `passport` 所組出的 URL 位址。而這個 URL 則是導向一個由 Google 架設的登入頁面。