...
在我們的 `setupGoogle` 函式內我們在認證成功時會呼叫 `verify` 函式。`verfiy` 會從 Google 伺服器取得所有需要的資訊，例如 `googleToken` 及 `profile`。在 `verify` 函式內我們呼叫 `User.signInOrSignUp`，這會回傳一個新增的使用者或是一個既有的使用者。我們另外還會在 `passport.deserializeUser` 內呼叫 `User.publicFields`。