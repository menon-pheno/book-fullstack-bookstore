...
3. 想像一下，假設有個使用者，登入了我們的網頁，關閉頁籤，重新開啟頁籤，他將看到他仍然是處於登入的狀態。這正是我們第二章提到的永久性登入的機制。快速重點複習，達到這個功能的方式是透過比對確認瀏覽器端的 `cookie` （解譯後）有對應的 `session` （存於我們 MongoDB 的 `Session` 文件）。