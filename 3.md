...
下一小節，我們稍微離題一下，討論 `this` 語法的意義。討論完畢後，我們回來在 Google Cloud Platform 上建立一個對應我們專案的網頁應用程式，並且測試我們整合 Google OAuth API 的成果。

---

#### this

如果你有在拜讀 Mongoose 的文件，你應該會看到幾個常見的語法，例如 `Model.findOne`、`Model.create`、`Model.updateOne` 等：

[http://mongoosejs.com/docs/api.html#create_create](http://mongoosejs.com/docs/api.html#create_create)

在我們的 `User.signInOrSignUp` 裡，我們透過 `this.create` 來呼叫 Mongoose 的 `create` API，而非使用 `User.create` 或是 `UserClass.create`。

為什麼呢？

在 JavaScript 世界裡，`this` 是一個保留字（keyword），它會指向一個 `context` 物件，而這個 `context` 物件是目前執行的程式碼的“擁有者”。有時候，一個函式會需要存取 `context` 這個物件內的資料，這時，該函式可以透過 `this` 去達成這個目標：
...
`printToConsole` 存取了 `user` 物件內的資料，這是透過 `this` 去參考了`user` 物件來達成的。
