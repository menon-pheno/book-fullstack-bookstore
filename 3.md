...
3. 
    ...
    當成功登入（也沒有進行登出）的使用者載入我們的頁面時，會發生甚麼事? 我們的伺服器需要將使用者資訊給**去序列化**並取出來。如果從瀏覽器來的 `cookie` 與 `session` 對應正確，且 `session` 存有使用者 id，那我們需要這個 id 取出並透過 `User.findById` 這個 Mongoose API 去確認這個使用者 id 有存在於 `User` MongoDB 集合裡。基於安全性的考量，我們只將 `public` 的欄位回傳給瀏覽器。
