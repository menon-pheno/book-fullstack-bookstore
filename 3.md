...
從第二章開始，我們就是在使用 `dotenv` 套件來管理環境變數。我們透過 `process.env.xxx` 這樣的方式來取得 `xxx` 這個環境變數的值。舉例來說，我們之前的程式碼裡的 `server/server.js` 的 `MONGO_URL` 就是用 `const MONGO_URL = process.env.MONGO_URL_TEST` 來設定。