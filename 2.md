...
這邊要表示的是，`Index.getInitialProps` 會回傳一個 props 值（`{ user: { email: 'default@bookstore.com' } }`），而這個 props 值會傳入 `Index` 元件內，你造訪 `http://localhost:3000` 的時候就會看到正確的 `user.email` 值被顯示。

在驗證了我們 `Index.getInitialProps` 會將正確的 prop 傳入 `Index` 頁面後，我們將會在 `Index.getInitialProps` 函式內呼叫 Web API 來取得該有的 `user` prop 值。我們將使用 `SWR` 這個套件來進行 Web API 的呼叫以及管理。我們透過在專案跟目錄下執行以下來安裝 `SWR`：
...
1. 使用者造訪 `ROOT_URL/`(在本地端就是造訪 `http://localhost:3000/`)
2. 伺服器端會嘗試渲染 `Index` 頁面(因為 `/` 路徑會對應到 `pages/index.jsx` 而這個檔案我們 `export` 就正是 `Index` 頁面元件)
3. `Index` 頁面元件會呼叫它的 `getInitialProps` 函式以取得 props 該有的值 - 以我們目前的頁面唯一需要的是 `<div>user prop(demo 用而已): {user.email || '沒資料'}</div>` 也就是 `user.email` 這個 prop 的值

所以我們下一步就是要在 `Index.getInitialProps` 裡透過 `SWR` 從 Web API 取得 user 的值，來當作 props。
