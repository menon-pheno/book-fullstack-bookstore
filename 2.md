...
我們來用 `Header` 元件當作範例。造訪一下[關於 Material-UI 的 App Bar 介紹](https://mui.com/components/app-bar/#main-content)。裡面的 `App Bar with menu` 範例我覺得相當符合我們 `Header` 元件的需求：

- 針對登入與否呈現不同的樣式（雖然我們還沒進行到實作登入的部分）
- 長相乾乾淨淨的

因為有了前面的準備，我們現在使用 Material-UI 的範例可以說是相當簡單，我們將 `components/Header.js` 改成以下：
...
這段程式碼是直接拷貝範例程式貼上，唯一改變的只有將範例內的 `MenuAppBar` 名稱改為 `Header` 以維持本來所 export 的名稱。讓我們逐步把範例繼續調整到我們想要的狀態。
