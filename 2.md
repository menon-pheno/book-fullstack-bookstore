...
將上述兩個步驟綜合起來，你的 `Index` 頁面元件會變成（我微調了一些顯示上的文字，讓首頁畫面呈現比較清楚點）：
...
現在 `Index` 頁面會透過 `Index.getInitialProps` 來取得頁面初始的 prop 值。一個很常見的動作，就是頁面會透過這個函式去呼叫一些 Web API 以取得頁面需要的資料。我們來定義一個 Web API 讓 `Index.getInitialProps` 可以使用，來示範一下這個過程。
...
我們在 `Index.getInitialProps` 目前的目標是要取得使用者的資訊，這樣就可以將資訊呈現在 `Index` 頁面上。有了一個 Web API 後，我們在 `Index.getInitialProps` 內呼叫這個 Web API 去取得使用者資訊，我們目前先簡單的在 Web API 這邊回傳一個寫死的使用者，晚點我們就會讓 Web API 去跟 MongoDB 資料庫溝通拿到存於資料庫的資訊。

我們將 `Index.getInitialProps` 修改成以下：
...
![alt user prop](./2-images/2-user-prop.png)
