...
依照我們前面講的，由於檔案位於 `pages/api` 之內，因此會被 Next.js 設定為一個 Web API，然後因為檔案本身名字為 `user`，因此此 Web API 的路由就會是 `ROOT_URL/api/user`（在我們專案中目前就會是 `http://localhost:3000/api/user`）。
