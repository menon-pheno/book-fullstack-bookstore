...
`.env.local` 是一個 Next.js 存放環境變數用的檔案（[Next.js 環境變數官方說明](https://nextjs.org/docs/basic-features/environment-variable#environment-variable-load-order)）。我們

---

### Next.js Web API

我們首先針對 `Index` 這個頁面元件做以下改動：

1. 先在頁面上做一些簡單的資料呈現，這是為了更好看清楚我們想要做到的事項
   - 使用 useState hook 來設定頁面的 state 值
2. 設定一個 Next.js Web API
   - 使用 useEffect hook 來呼叫 Web API
3. 安裝 `SWR` 套件
   - 使用 `swr` 來透過 Web API 抓資料
...
1. 我們先將 `Index` 頁面改成以下：
   ...
   - `import { useState } from 'react'`：因為我們現在先假定 `Index` 頁面要有一個 `user` 的 state 物件
   - `const [user] = useState({ email: 'default@bookstore.org' })`：這是 react 的 state hook，可以用來管理 react 元件的 state，我們給了這個 state 一個預設值 - `{ email: 'default@bookstore.org' }`
   - 我們移除了第一章使用的 Material-UI Button
   - 在我們呼叫了上面的 `useState` 之後，`Index` 這個 react 元件就會含有一個 `user` 的 state 了，我們透過 `使用者 email: {user.email}` 來驗證這個 state 存在，並且有正確的預設值（`default@bookstore.org`）

![alt user state](./2-images/2-user-state.png)

2. 目前 `Index` 頁面會透過 `useState` 來取得頁面初始的 state 值。一個很常見的動作，就是頁面會透過呼叫一些 Web API 以取得頁面需要的資料（來設定正確的 state）。我們先來示範如何定義一個 Web API。

   Next.js 提供了一個統一的方式讓你設定 Web API。任何放在 `pages/api` 內的檔案會被對應到 `/api/*` 路由的一個 API。值得注意的是，這些 API 路由都會存在於伺服器端，並不會增加瀏覽器端的負擔。讓我們用個例子來說明。

   新增一個 `pages/api/user.js`，內容如下：
   ...
   依照我們前面講的，由於檔案位於 `pages/api` 之內，因此會被 Next.js 設定為一個 Web API，然後因為檔案本身名字為 `user`，因此此 Web API 的路由就會是 `ROOT_URL/api/user`（在我們專案中目前就會是 `http://localhost:3000/api/user`）。如果你嘗試在瀏覽器造訪 `http://localhost:3000/api/user`：

![alt user api](./2-images/2-user-api.png)

    在 Next.js 的架構下，基本架設 Web API 的方法重點有以下：

- `pages/api/xxx.js` 會對應到 `ROOT_URL/api/xxx` 這個 API 路由
- `pages/api/xxx.js` 檔案內要 `export default` 一個函式，通常會叫這個函式為請求處理函式（request handler function）
- 這個處理函式會傳入兩個參數：`req` 及 `res`
- `req` 會是一個 `http.IncomingMessage` 的實體
- `res` 會是一個 `http.ServerResponse` 的實體

  有了一個 Web API 後，我們就可以在 `Index` 內呼叫這個 Web API 去取得使用者資訊，我們目前先簡單的在 Web API 這邊回傳一個寫死的使用者，晚點我們就會讓 Web API 去跟 MongoDB 資料庫溝通拿到存於資料庫的資訊。

  我們先將 `pages/api/users.js` 修改成以下：

  ```JavaScript
  export default function handler(req, res) {
    // 只是單純將 email 屬性改為 default2@bookstore.org, 比較好做比照
    res.status(200).json({ email: "default2@bookstore.org" });
  }
  ```

  我們這邊先示範一下使用瀏覽器內建的 `fetch` 加上 react 內建的 `useState` 及 `useEffect` 兩個 hook 如何進行 Web API 的呼叫。將 `/pages/Index.jsx` 修改為如下：
  ...
  - 實際呼叫 Web API 的過程在 `useEffect` 裡面（也就是 `fetch`）
  - 在得到 `api/user` 所回傳的資料後，`useEffect` 內會呼叫 `setUser` 來更新 `user` state 的資料

  ![alt user api](./2-images/2-user-fetch.png)

  假設這時 `yarn dev`，你會看到頁面有一個很短暫的時間顯示 `載入中`，然後塵埃落定的時候，會看到上面的截圖，代表我們正確地從 `api/user` 這個位址拿到 `user` 的更新值，並且將之設定到 `user` state 裡。

3. 以上的方法當然是可行的（我們也實作證明了），不過根據 Next.js 團隊的高度建議使可以使用他們所開發的 `SWR` react hook 套件，我們自己使用起來的經驗也的確認為這個 `SWR` 套件相當好用，所以就依循他們的建議，我們來將目前的 `Index` 頁面改使用 `SWR`。先來安裝

   ```
   yarn add swr
   ```

   將 `pages/index.jsx` 的程式改為以下：
   ...
   - 不再需要 `useState` 及 `useEffect` 了，都由 `useSWR` 給包辦了（程式碼會相對比較簡潔）
   - `useSWR` 取得的資料都會放在 `data` 屬性內，如果 Web API 有錯誤時，會記錄在 `error` 屬性內（我們未來會調整使用 `SWR` 的方式，目前先以最基本原始的使用法，讓大家先熟悉這個套件）
   - 我們在呼叫 `useSWR` 時，第二個參數提供了一個抓取資料的 `fetcher`，這個 `fetcher` 的實作在我們的範例裡面是使用 `fetch`，但是如果需要或是想要的話可以任意替換成你自己喜歡的抓取資料的套件或方式

   這邊我們做個簡單的重構，因為我們可以想見，接下來其他的頁面也很可能會有透過 Web API 取得資料的需求，所以我們把 `const fetcher = (...args) => fetch(...args).then((res) => res.json());` 抽取到一個模組裡，這樣其他的元件也可以使用。

   新增一個 `lib/dataFetcher.js`：
...
開始之前，我們先安裝 `mongoose` 套件：

```
yarn add mongoose@5.10.0
```

在之前的小節，我們透過定義 Web API 來提供 `Index` 頁面所需要的資訊。我們將 `user` 物件在伺服器的 Web API 裡寫死。在這個小節，我們要使用 `mongoose` 套件裡的 `mongoose.model` 函式來定義我們的第一個 `User` 資料模型：
...
綜合以上的說明，新增 `/models/User.js`（新增 `models` 這個資料夾），內容如下：
...
這樣一來，我們就可以從 MongoDB 資料庫取得 `User` 文件了。假設我們的資料庫裡面有個 `User` MongoDB 文件，而且它的 `slug` 值是 `super-author`，那我們可以使用 Mongoose API 的 `findOne` 函式來從資料庫取得這筆資料：
...
在我們建立與 MongoDB 連線之前，讓我們先來把目前的 Next.js 升級一下：

- 這樣我們可以先看一下一般套件是如何升級
- 趁著目前專案還不複雜的時候升級比較輕鬆，比較不用擔心某些複雜的功能因為升級壞掉
- 我們會使用 Next.js 提供的環境變數功能（而 Next.js 的環境變數功能在 9.4 版後做了一些調整）

我們這次升級的目標是將 Next.js 升到它目前的最新 production 版本（Next.js v12）。首先，我們將 node 升級（這是因為 Next.js v12 要求 node 版本要在 v12.22 以上），我們就趁機將 node 升級到最新的 lts 版本：
...
好了，讓我們繼續吧。到這邊，我們有一個 `User` 資料模型，但是我們還沒有將伺服器與 MongoDB 的資料庫連接起來。使用 `mongoose` 來與 MongoDB 連接很單純，你傳入 `uri` 以及 `options` 用來呼叫 `mongoose.connect` 就行了：
...
我們建立一個會進行以上動作的檔案，`/lib/dbConnector.js`（本質上就是建立 MongoDB 的連線，但是我加上了一些註解）：

```JavaScript
import mongoose from 'mongoose';

// 從環境變數取得 MongoDB 的 URI 位址
...
// cached 最主要的功能是，在開發環境內，我們有新的程式碼的時候，會進行 hot reload
// 而我們將本來有連接到 MongoDB 的資訊給 cache 起來，這樣 hot reload 的程式碼就可以繼續使用，而不用進行新的 MongoDB 連結
...
async function dbConnect() {
  // 如果 cache 內已經有 connection, 就可以直接回傳了
  if (cached.conn) {
    return cached.conn;
  }
...
寫到這邊，我們有了一個 `User` 資料模型，也有了一個對 MongoDB 連線的函式，現在則是在我們的 `api/user` 裡做對應的動作，之前是寫死，但是現在應該是透過 `dbConnector` 來從 MongoDB 查詢 `User` 該有的值。

`pages/api/user.js` 會變成以下：
...
如果這時你 `yarn dev` 並造訪 `http://localhost:3000`：

![alt user api](./2-images/2-user-error.png)

而在終端機可以看到：

![alt mongo env](./2-images/2-terminal-env-error.png)

所以只差一點點了，現在差的是建立一個 MongoDB，將 `MONGODB_URI` 找出來設定到環境變數內，這樣路就串通了。
