...
我們接著來實作 `Chapter` 資料模型（完成後會回頭稍微調整 `models/Book.js`，因為 `Book` 與 `Chapter` 是有關聯性的）。

---

## `Chapter` 資料模型

在我們的書店裡，書本是由多個章節（`Chapter`）來組成的。我們在本章後面會實作一個 `ReadChapter` 頁面，它將會把單一章節的內容給渲染出來。在我們定義完 `Chapter` 資料模型後，我們將會回頭在剛剛實作的 `Book` 模型內把書本接上該書本應該具備的 `Chapter`。
...
[Client-side data fetching with SWR](https://nextjs.org/docs/basic-features/data-fetching/client-side)

yarn add swr

lib/dataFetcher.js

delete /pages/api/books/[...bookInfo].js

create 2 books on MongoDB Atlas

/pages/index.js

see two books on index.js
