...
- content: "
  ...
  - 上面這段可能會讓人有點困惑，但是主要是要在 `bookstoreㄡchapters` 的 `chapter.content` 這個屬性放入一個 markdown 的文件，上面是 markdown 的一些常見的語法
  - 請把 `{三個 back tick}` 的部分換成 ```，這是 markdown 表示程式碼的方式

新增 pages/books/[...slugs].js：
...
Move Header to \_app.js
delete Header in pages/index.js

rename Chapter htmlContent to mdContent => or delete altogether

delete api/chapterIndexTest.js
delete api/test.js
